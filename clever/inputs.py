"""clever.inputs — static model input tables (no scenario dependency).

Country/technology mappings, EOLES cost & lifetime tables, PEMMDB hydro capacities,
VRE specs & profile fallbacks, BESS base specs, manual interconnection NTCs, and the
numerical/default scalars. Every value here is a plain literal with no dependency on the
scenario string — scenario-gated derived tables live in clever/constants.py (the
override layer), and the scenario-string parsers live in clever/scenario/parse.py.

Carved verbatim out of clever/constants.py; values are byte-for-byte identical
