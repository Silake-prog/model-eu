...
def get_writable_data_path() -> Path:
    ...
    Priority order:
      1. ``POMMES_EUR_DATA`` (or legacy ``CLEVER_DATA``) environment variable, if set.
      2. ``<package_dir>/results`` (if writable).
      3. ``~/.local/share/clever`` (always writable).
    """
    # 1. Environment variable override (POMMES_EUR_DATA wins; CLEVER_DATA legacy)
    env = os.environ.get("POMMES_EUR_DATA") or os.environ.get("CLEVER_DATA")
    if env:
        p = Path(env)
